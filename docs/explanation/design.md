...
*Why `@voxgig/build` works the way it does.*
...
- **Application code** (the EnvWeb SPA, runtime environment entries, seed
  data, custom views) is a *starting point* that developers take over.
  These are create-once: generation skips existing files, so hand edits
  are safe. The cost is that framework updates don't flow into existing
  files automatically — the escape hatches are `force` regeneration and
...
an entity graph of hundreds of entities, and adapt to a model change with
a reload rather than with regeneration churn and merge conflicts in
generated UI code.
...
The customisation story mirrors the Seneca priors of the backend (override
an action without editing it) in the frontend:
