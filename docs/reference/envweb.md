...
*The generated web app: inputs, outputs, and extension points.*
...
| `env` | The model environment entry: `dir`, `port`, `users`, `e2eport` (optional) |
...
- **Create-once** (the default for every file in `WEB_FILES`): if the
  ...
  framework updates require a forced regeneration or manual merge.
...
| Slot | Value |
|---|---|
| `name` / `Name` | `main.conf.core.name`, as written, and in camel case |
| `fe` | the frontend directory: the environment's `dir`, default `web` |
| `users` | JSON of the seed users (the environment's `users`, or built-in defaults) |
| `seedEmail` / `seedPassword` | first seed user's credentials |
| `e2eport` | the environment's `e2eport`, default backend port + 10 |
...
- **Docs**: an agent guide for the web app, six documentation pages
  under `docs/` (see the file manifest), and a doc sidecar per component
...
- **Backend** `backend/src/`: `env/web/web.ts` (Express + gateway runner
  — also starts a development REPL on `conf.port.repl`; `REPL=false`
  disables), `env/shared/seed.ts` (create-once seed stub), `srv/auth/*`
  (`signin`, `signout`, `load`, `change-pass`, `update-user`,
  `remind-pass` + `web_` wrappers), `srv/ent/*` (generic CRUD:
  `cmd_list/load/save/remove`, `access.ts` membership scoping),
  `srv/api/*` (the REST API service: `on_ent.ts` + `expose.ts`;
  `valid_gen.ts` is REGENERATED by `api_gen`), API-key management
  (`srv/auth/*apikey*`), and `env/web/api.ts` (the strict-JSON HTTP
  mapping at `main.api.prefix`, Bearer API-key auth).
...
`theme.js` persists the chosen mode (the `localStorage` key
`vg-theme-mode`) and applies it via the `data-theme-mode` attribute; its
mode list passes through the `theme:modes` filter hook.
