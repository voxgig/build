...
const GENMARK = (what) => `<!-- AUTO-GENERATED from the model by @voxgig/build (${what}) - do not edit. -->`;
        ...
        for (const k of Object.keys(inaim)) {
            if ('req' === k) {
                ...
                for (const rk of Object.keys(on)) {
                    for (const entry of (0, util_1.aimmsgs)(msg, 'req')) {
                ...
                for (const entry of (0, util_1.aimmsgs)(msg, k)) {
                    own.push({
...
function entitiesMd(model) {
    ...
    lines.push('|---|---|---|---|');
    for (const e of ents) {
        const flds = e.fields.map((f) => f.name).join(', ');
        const refs = e.refs.map((r) => `${r.field} → ${r.target}`).join('<br>') || '—';
        lines.push(`| \`${e.canon}\` | ${flds} | ${refs} | ${e.custom ? 'custom view' : 'generic admin'} |`);
    }
    lines.push('');
    return lines.join('\n');
}
function messagesMd(model) {
...
const doc_gen = async (model, spec) => {
    const root = spec.root;
    const srvfolder = spec.srvfolder || path_1.default.join(root, 'backend', 'src', 'srv');
    const created = [];
    const refdir = path_1.default.join(root, 'docs', 'reference');
    if (writeIfChanged(path_1.default.join(refdir, 'entities.md'), entitiesMd(model))) {
        created.push('docs/reference/entities.md');
    }
    if (writeIfChanged(path_1.default.join(refdir, 'messages.md'), messagesMd(model))) {
        created.push('docs/reference/messages.md');
    }
    if (writeIfChanged(path_1.default.join(refdir, 'system-map.md'), systemMapMd(model))) {
    ...
    for (const s of services(model)) {
        const dir = path_1.default.join(srvfolder, s.name);
        if (fs_1.default.existsSync(dir)) {
            if (writeIfChanged(path_1.default.join(dir, 'README.md'), srvReadmeMd(model, s))) {
